...
return {
    entry = function()
        ...
        -- Get input for mode selection
        local mode, event = ya.input({
            title = "Select mode: (1) Single archive, (2) Separate archives",
        ...
        elseif mode == "2" then
            ...
            for path, names in pairs(path_fnames) do
                ...
                while true do
                    if file_exists(output_url) then
                        local overwrite_answer = ya.input({
                            title = "Overwrite " .. individual_output_name .. "? y/N:",
                            position = { "top-center", y = 3, w = 40 },
                        })
                        if overwrite_answer:lower() ~= "y" then
                            notify_error("Operation canceled", "warn")
                            ...
                            local rm_status, rm_err = os.remove(output_url)
                            if not rm_status then
                                notify_error(string.format("Failed to remove %s, exit code %s", individual_output_name, rm_err), "error")
                                return
                            end -- If overwrite fails, exit
                        end
                    end
                    if archive_compress and not individual_output_name:match("%.tar$") then
                        individual_output_name = individual_output_name:match("(.*%.tar)") -- Test for .tar and .tar.*
                        output_url = combine_url(output_dir, individual_output_name) -- Update output_url
                    else
                        break
                    end
                end
                ...
                if archive_compress then
                    local compress_status, compress_err =
                        Command(archive_compress):args(archive_compress_args):arg(individual_output_name):cwd(output_dir):spawn():wait()
                    if not compress_status or not compress_status.success then
                        notify_error(
                            string.format(
                                "%s with %s failed, exit code %s",
            ...
            notify_error("Invalid mode selected", "error")
        end
    end,
}